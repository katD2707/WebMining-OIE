...
def sample_exemplars(training_exemplar_df,num_exemplars=3):
    ...
    return ''.join([f"input:\nS: {exemplar_sample.iloc[i]['question']}\nA: <INS>\noutput:\n{exemplar_sample.iloc[i]['answer']}\n" for i in range(num_exemplars)])
...
def compute_score(anno_path, extraction_path):
    gold_annotation_file = anno_path
    openie6_extractions_file = extraction_path
    ...
    # Compute scores
    benchie.compute_f1()
    return benchie.scores['openie6'].f1

def score_prompts(scorer_chain,prompts,training_examples,performance_df):
    for prompt in prompts:
        with open("gpt_extraction.txt", "w") as ext, open("gold_anno.txt", "w") as anno:
            for index, example in training_examples.iterrows():
                question = example['question']
                gold_anno = example['gold_anno']
                sample_answer = scorer_chain.predict(question=question,instruction=prompt)
                sample_answer = ["\t".join(sent.strip().replace("(","").replace(")", "").split(";"))
                                 for sent in sample_answer.split("\n")]
                for sample in sample_answer:
                    ext.write(f"{index}\t{sample}\n")
                anno.write(f"{gold_anno}\n")
                # scores.append(are_numbers_the_same(answer,sample_answer))
        scores = compute_score("gold_anno.txt", "gpt_extraction.txt")
        score = int(100*scores)
...
def opro(optimizer_chain,scorer_chain,performance_df,training_exemplar_df,n_scores=20,n_exemplars=3,n_prompts=8,n_training_samples=10,max_iterations=3):
    ...
    for _ in range(max_iterations):
        texts_and_scores = build_text_and_scores(performance_df)    # Fine
        exemplars = sample_exemplars(training_exemplar_df,n_exemplars) # "input: S: <sent> A: <INS> output: <answer>"
